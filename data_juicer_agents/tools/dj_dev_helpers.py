...
def get_basic_files() -> ToolResponse:
    ...
    try:
        combined_content = f"# DataJuicer Path: {DATA_JUICER_PATH}\n"
...
async def get_operator_example(
    ...
    Returns:
        ToolResponse: Example operator code and test files for the specified operators
    """
        ...
        if not operator_names:
            return ToolResponse(
                content=[
                    TextBlock(
                        type="text",
                        text="No operator names provided. Please provide a list of operator names to get examples.",
                    ),
                ],
            )

        combined_content = f"# Operator Examples\n\n"
        combined_content += f"Requested operators: {', '.join(operator_names)}\n\n"

        # Process each operator name
        for i, tool_name in enumerate(operator_names):
