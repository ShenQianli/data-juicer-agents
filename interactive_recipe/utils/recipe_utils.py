...
class RecipeManager:
    ...
    def _load_all_recipes(self) -> list[Recipe]:
        ...
        if not os.path.isdir(self.recipes_dir):
            logger.warning(f"Recipes directory not found: {self.recipes_dir}, Auto loading...")
            self._clone_recipes_from_github()
    ...
    def _clone_recipes_from_github(self):
        """Clone the data-juicer-hub repository to get recipes."""
        try:
            repo_url = "https://github.com/datajuicer/data-juicer-hub.git"
            clone_dir = "data-juicer-hub"
            
            logger.info(f"Cloning recipes from {repo_url}...")
            ...
            # Check if the recipes directory exists in the cloned repo
            cloned_recipes_path = os.path.join(clone_dir, "recipes")
            if os.path.isdir(cloned_recipes_path):
                # Update recipes_dir to point to the cloned recipes
                self.recipes_dir = cloned_recipes_path
                logger.info(f"Successfully cloned recipes to {self.recipes_dir}")
            else:
                # If no recipes directory in the cloned repo, use the clone_dir itself
                self.recipes_dir = clone_dir
                logger.info(f"Using cloned repository directory: {self.recipes_dir}")
                
        except subprocess.CalledProcessError as e:
            logger.error(f"Failed to clone repository: {e.stderr}")
            logger.warning(f"Continuing with original recipes directory: {self.recipes_dir}")
